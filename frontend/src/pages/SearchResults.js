    ...
    <div style={{ background: '#fce4ec', minHeight: '100vh', padding: '16px 0' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          <button
            onClick={() => setShowFilters(!showFilters)}
            style={{ background: '#e91e63', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 4, fontSize: 13, fontWeight: 'bold', cursor: 'pointer' }}
          >
          ...
          <select
            value={sort}
            onChange={e => handleSortChange(e.target.value)}
            style={{ flex: 1, padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: 4, fontSize: 13 }}
          >
            {SORTS.map(s => <option key={s.v} value={s.v}>{s.l}</option>)}
          </select>
        </div>
  ...
  >
              ...
              {c !== 'All' && (() => {
                const count = products.filter(p =>
                  p.category === c ||
                  (p.categories && p.categories.includes(c))
                ).length;
                return count > 0 ? <span style={S.catCount}>{count}</span> : null;
              })()}
                  </div>
                ))}
              </div>
